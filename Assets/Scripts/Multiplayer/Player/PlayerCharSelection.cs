...
namespace Multiplayer.CharacterSelection
{
    public class PlayerCharSelection : NetworkBehaviour
    {
        ...
        private void Start()
        {
            ...
            gameObject.name = $"Player{m_playerId.Value}";
        }
        ...
        private void ChangeCharacterSelection(int value)
        {
            ...
            if (charTemp >= CharacterSelectionManager.Instance.charactersData.Count)
                charTemp = 0;
            else if (charTemp < 0)
                charTemp = CharacterSelectionManager.Instance.charactersData.Count - 1;
